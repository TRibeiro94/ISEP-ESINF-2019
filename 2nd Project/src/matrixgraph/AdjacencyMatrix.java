...
public class AdjacencyMatrix {
    ...
    */
	...
	vertices = new ArrayList<Pais>(initialSize);
    ...
    */
    public boolean checkVertex(Pais vertex) {
    ...
    */
    @SuppressWarnings("unchecked")
    public Iterable<Pais> vertices() {
	return (Iterable<Pais>) vertices.clone();
    }
     ...
     */
    public int outDegree(Pais vertex) {
     ...
     */
    public int inDegree(Pais vertex) {
     ...
     *         exist in the graph.
     */
    public Double getEdge(Pais vertexA, Pais vertexB) {
     ...
     *         graph.
     */
    public Pais[] endVertices(Double edge) {
        for (int i = 0; i < numVertices - 1; i++)
            for (int j = i + 1; j < numVertices; j++)
                if (edgeMatrix[i][j] != null)
                    if (edgeMatrix[i][j].equals(edge)) {
                        @SuppressWarnings("unchecked")
                        Pais[] result = (Pais[]) new Object[2];
     ...
     */
    public boolean insertVertex(Pais newVertex) {
    ...
    public boolean insertEdge(Pais vertexA, Pais vertexB, Double newEdge) {
     ...
     */
    public boolean removeVertex(Pais vertex) {
        int index = toIndex(vertex);
        if (index == -1)
            return false;
     ...
     *         connected
     */
    Double removeEdge(int indexA, int indexB) {
        Double edge = edgeMatrix[indexA][indexB];
        edgeMatrix[indexA][indexB] = edgeMatrix[indexB][indexA] = null; // undirected graph
        numEdges--;
        return edge;
    }		

    public Double removeEdge(Pais vertexA, Pais vertexB) {
        int indexA = toIndex(vertexA);
        if (indexA == -1)
                return null;
